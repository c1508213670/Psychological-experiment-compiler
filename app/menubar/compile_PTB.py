...
def printCycleWidget(cWidget, f, attributesSetDict, cLoopLevel, allWidgetCodes):
    ...
    nowmyls=[i for i in range(cWidget.rowCount())]
    if cycleOrderStr=="'Random without Replacement'":
        random.shuffle(nowmyls)
    elif cycleOrderStr=="'Random with Replacement'":
        for i in range(cWidget.rowCount):
            nowmyls[i]=random.randint(0,cWidget.rowCount-1)
    print(nowmyls)
    for iRow in nowmyls:
        cRowDict = cWidget.getAttributes(iRow)
        print(cRowDict)
...
def drawSoundWidget(cWidget, soundStimCodes, attributesSetDict, cLoopLevel, allWidgetCodes, cProperties=None, iSlave=1):
    ...
    durationdist = dict()
    durationdist = genCheckResponse(cWidget, soundStimCodes, cLoopLevel, attributesSetDict, durationdist)
    cWidgetName = getWidgetName(cWidget.widget_id)
    print(durationdist)

    if durationdist:
        corRespStr = durationdist["corRespStr"]
    else:
        corRespStr = "[]"
    ...
    if corRespStr == "[]":
        if not durationdist:
            ...
            if is_number(durStr):
                ...
                printAutoInd(soundStimCodes, "trial_duration: {0},", durStr)
            else:

                printAutoInd(soundStimCodes, "trial_duration: {0},", "`${"+str_html_var(durStr)+"}`")
        else:
            # 获取duration中的值
            allowableKeysStr = durationdist["allowableKeysStr"]
            ##########################################################print(allowableKeysStr)
            rtWindowStr = durationdist["rtWindowStr"]

            endActionStr = durationdist["endActionStr"]
            printAutoInd(soundStimCodes, "    var {0} = ", cPrefixStr)
            soundStimCodes.append("{")
            printAutoInd(soundStimCodes, "type: 'audio-keyboard-response',")
            printAutoInd(soundStimCodes, "  stimulus: {0},", mypathform(cFileNameStr))
            if allowableKeysStr != "1:255":
                allowableKeysStr = changekeystr(allowableKeysStr)
                if allowableKeysStr.find("var")!=-1:
                    printAutoInd(soundStimCodes, 'choices: {0},', "`${"+str_html_var(allowableKeysStr)+"}`")
                else:
                    printAutoInd(soundStimCodes, 'choices: {0},', allowableKeysStr)

            # printAutoInd(f, 'key_answer: "a",')

            if is_number(durStr):
                printAutoInd(soundStimCodes, "stimulus_duration: {0},", durStr)
            if rtWindowStr == "-1" or rtWindowStr == "-2":
                if is_number(durStr):
                    printAutoInd(soundStimCodes, "trial_duration: {0},", durStr)
                else:
                ...
                printAutoInd(soundStimCodes, "trial_duration: {0},", rtWindowStr)

            if endActionStr == "0":
                printAutoInd(soundStimCodes, "response_ends_trial: false,")
        ...
        if allowableKeysStr != "1:255":
            ...
            if allowableKeysStr.find("var")!=-1:
                ...
                printAutoInd(soundStimCodes, 'choices: {0},', allowableKeysStr)
        if corRespStr == "1:255":
            printAutoInd(soundStimCodes, "key_answer: 'ALL_KEYS',")
        else:
            corRespStr = changekeystr(corRespStr)
            printAutoInd(soundStimCodes, 'key_answer: {0},', corRespStr[1:len(corRespStr) - 1])
        if is_number(durStr):
            printAutoInd(soundStimCodes, "stimulus_duration: {0},", durStr)
        if rtWindowStr == "-1" or rtWindowStr == "-2":
            if is_number(durStr):
...
def drawImageWidget(cWidget, f, attributesSetDict, cLoopLevel, allWidgetCodes, cProperties=None, cVSLCodes=None):
    ...
    durationdist = dict()
    durationdist = genCheckResponse(cWidget, f, cLoopLevel, attributesSetDict, durationdist)
    cWidgetName = getWidgetName(cWidget.widget_id)
    print(durationdist)
    ...
    if corRespStr == "[]":
        if not durationdist:
            ...
            if is_number(durStr):
                printAutoInd(f, "trial_duration: {0},", durStr)
            else:

                printAutoInd(f, "trial_duration: {0},", "`${"+str_html_var(durStr)+"}`")
        else:
            # 获取duration中的值
            allowableKeysStr = durationdist["allowableKeysStr"]
            ##########################################################print(allowableKeysStr)
            ...
            if allowableKeysStr != "1:255":
                allowableKeysStr = changekeystr(allowableKeysStr)
                if allowableKeysStr.find("var")!=-1:
                    printAutoInd(f, 'choices: {0},', "`${"+str_html_var(allowableKeysStr)+"}`")
                else:
                    printAutoInd(f, 'choices: {0},', allowableKeysStr)
            # printAutoInd(f, 'key_answer: "a",')

            if is_number(durStr):
                printAutoInd(f, "stimulus_duration: {0},", durStr)
            if rtWindowStr == "-1" or rtWindowStr == "-2":
                if is_number(durStr):
                    printAutoInd(f, "trial_duration: {0},", durStr)
                else:
                ...
                printAutoInd(f, "trial_duration: {0},", rtWindowStr)

            if endActionStr == "0":
                printAutoInd(f, "response_ends_trial: false,")
        ...
        if allowableKeysStr != "1:255":
            ...
            if allowableKeysStr.find("var")!=-1:
                ...
                printAutoInd(f, 'choices: {0},', allowableKeysStr)
        if corRespStr == "1:255":
            printAutoInd(f, "key_answer: 'ALL_KEYS',")
        else:
            corRespStr = changekeystr(corRespStr)
            printAutoInd(f, 'key_answer: {0},', corRespStr[1:len(corRespStr) - 1])
        if is_number(durStr):
            printAutoInd(f, "stimulus_duration: {0},", durStr)
        if rtWindowStr == "-1" or rtWindowStr == "-2":
            if is_number(durStr):
...
def drawTextWidget(cWidget, f, attributesSetDict, cLoopLevel):
    ...
    fontNameStr, isFontNameRef = getRefValue(cWidget, cProperties['Font Family'], attributesSetDict)
    fontName = dataStrConvert(fontNameStr, isFontNameRef)
    if fontName.find("var")!=-1:
        fontName="${"+str_html_var(fontName)+"}"
    else:
        fontName=fontName[1:-1]

    fontSizeStr, isFontSizeRef = getRefValue(cWidget, cProperties['Font Size'], attributesSetDict)
    fontSize = dataStrConvert(fontSizeStr, isFontSizeRef)
    if str(fontSize).find("var")!=-1:
        fontSize="${"+str_html_var(fontSize)+"}"
    fontStyleStr, isFontStyleRef = getRefValue(cWidget, cProperties['Style'], attributesSetDict)
    fontStyle = dataStrConvert(fontStyleStr, isFontStyleRef)
    fontStyle = parseFontStyleStr(fontStyle)
    ...
    drawFrameEffect(cWidget, f, cProperties, attributesSetDict)  # frame内容的编译
    durationdist=dict()
    durationdist=genCheckResponse(cWidget, f, cLoopLevel, attributesSetDict,durationdist)
    cWidgetName = getWidgetName(cWidget.widget_id)
    print(durationdist)
    ...
    if corRespStr=="[]":
        if not durationdist:
            ...
            printAutoInd(f,
                         "<div style='background:{2};display: flex;flex-direction: row;flex-wrap: wrap;justify-content: {0};align-items: {1};'>",
                         alignmentX, alignmentY, fontBkColor)
            printAutoInd(f, "<p><font style=\"color:{2}\" size=\"{0}\" face=\"{1}\" >{3}</font></p></div>", fontSize,
                         fontName, addedTransparentToRGBStr(fontColorStr, fontTransparent), cTextContentStr)
            printAutoInd(f, "  `,")
        ...
        printAutoInd(f, "    var {0} = ", cWidgetName)
        f.append("{")
        printAutoInd(f, "type: 'categorize-html',")
        printAutoInd(f, "  stimulus:  `")
        printAutoInd(f,
                     "<div style='background:{2};display: flex;flex-direction: row;flex-wrap: wrap;justify-content: {0};align-items: {1};'>",
                     alignmentX, alignmentY, fontBkColor)
...
def compileCode(isDummyCompile):
    ...
    with open(finalMfilename, "w", encoding=encodingFormat) as f:
        ...
        printAutoInd(f, "    <script src=\"jspsych-6.3.0/plugins/jspsych-fullscreen.js\"></script>")
        ...
        printAutoInd(f, "    <script src=\"jspsych-6.3.0/plugins/jspsych-categorize-html.js\"></script>")
        printAutoInd(f, "    <script src=\"jspsych-6.3.0/plugins/jspsych-preload.js\"></script>")
        ...
        printAutoInd(f,"var mypreload =")
        f.write("{\n")
        printAutoInd(f,"    type: 'preload',")
        printAutoInd(f,"    auto_preload: true")
        f.write("};\n")
        printAutoInd(f,"timeline.push(mypreload);")
        printAutoInd(f,"var enter_fullscreen =")
        f.write("{\n")
        printAutoInd(f,"    type: 'fullscreen',")
        printAutoInd(f,"    fullscreen_mode: true")
        f.write("};\n")
        printAutoInd(f,"timeline.push(enter_fullscreen);")
        ...
        printAutoInd(f, "on_finish: function()")
        f.write("{\n")
        printAutoInd(f, "  jsPsych.data.displayData();")
        f.write("}\n")
        f.write("});\n  </script>\n</html>")
