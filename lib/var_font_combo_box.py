...
class VarFontComboBox(QFontComboBox):
    ...
    def __init__(self, editable: bool = True, parent=None):
        super(VarFontComboBox, self).__init__(parent)
        self.setAcceptDrops(True)
        self.setEditable(editable)
        self.setInsertPolicy(QFontComboBox.NoInsert)
    ...
    def setReg(self, reg_exp: str or list or tuple):
        if isinstance(reg_exp, str):
            self.reg_exp = f"{reg_exp}|{VarFontComboBox.Attribute}"
        elif isinstance(reg_exp, list) or isinstance(reg_exp, tuple):
            self.reg_exp = f"{'|'.join(reg_exp)}|{VarFontComboBox.Attribute}"
        self.setValidator(QRegExpValidator(QRegExp(self.reg_exp), self))

    def focusOutEvent(self, e):
        self.focusLost.emit()
        QFontComboBox.focusOutEvent(self, e)
